...
const JOURNAL_API_URL = '/api/journals/';
...
function printJournalMap(journal) {
  ...
  for (let i in journal.logs) {
    ...
    let popupTextext = `<div class="marker-date">${formatDate(
      log.createdAt
    )}</span></div>
        <div class="marker-coordinates">${formatCoordinate(
          log.coordinates.latitude
        )}° / ${formatCoordinate(log.coordinates.longitude)}°</div>`;
